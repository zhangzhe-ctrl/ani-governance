...
#     - api/buf.model.gen.yaml    Model 切片，显式钉 protoc-gen-go-grpc@v1.6.0
#     - api/buf.network.gen.yaml  Network 切片，同上
#   因此全量生成会覆盖切片产出，产生与本次改动无关的差异。
#
# 两类噪声（都无功能影响，但会污染 review）：
#   1) 版本/命名漂移：i_model_grpc / i_network_grpc 的版本头，
#      以及 i_tenant / i_user / i_task / i_server_monitor 的 http handler
#      序号（protoc-gen-go-http 按方法名全局计数，输入集变化即漂移）。
#      → 直接还原为 HEAD 版本，保持与既有生成物一致。
#   2) 空壳 validate：i_model / i_network / catalog 的 *.pb.validate.go
...
#
# 注意：本脚本只处理"必然噪声"。若你确实有意改动 Model/Network 切片，
# 请改用对应的 scripts/generate-*-slice.sh，不要依赖本脚本。
...
# 1) 还原版本/命名漂移文件
DRIFT_FILES=(
  "$GEN/admin/service/v1/i_model_grpc.pb.go"
...
NOOP_VALIDATE=(
  "$GEN/admin/service/v1/i_model.pb.validate.go"
  "$GEN/admin/service/v1/i_network.pb.validate.go"
  "$GEN/catalog/service/v1/model.pb.validate.go"
