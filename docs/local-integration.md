# Governance ↔ 业务服务本机联调历史记录（2026-09-20）

本文件保留当时 Network 联调的故障背景，**不是当前部署操作手册**。2026-09-21 起已移除启动自动迁移和播种，API 同步改为增量模式。新部署、首管理员、套餐、升级和登录验收一律按 [部署与初始化流程](deployment.md)；不要执行本历史记录中的旧初始化步骤或默认密码补种。
