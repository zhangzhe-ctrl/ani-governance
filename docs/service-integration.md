...
- **Model（强制依赖）**：[i_model.proto](../api/protos/admin/service/v1/i_model.proto) + [model_client.go](../app/admin/service/internal/data/model_client.go) + [model_service.go](../app/admin/service/internal/service/model_service.go)。地址未配置则启动失败。
- **Network（可选依赖）**：[i_network.proto](../api/protos/admin/service/v1/i_network.proto) + [network_client.go](../app/admin/service/internal/data/network_client.go) + [network_service.go](../app/admin/service/internal/service/network_service.go)。地址为空则跳过客户端装配。

新接入优先按 Model 模式走；仅当下游在目标环境可能缺席时才用 Network 的可选模式。
...
| 仅编译 | `make build_only` |
| 依赖与模块 | `go mod tidy`（固定版本模块按 §0.3 核对交付方式） |
...
3. **下游 Go API 模块交付**。出站客户端 import 下游仓库的生成包（如 `github.com/zhangzhe-ctrl/ani-model-service/api/model/v1`）。固定版本模块需要匹配的交付与 file-GOPROXY 配置，参照 [package-model-contract.py](../scripts/package-model-contract.py) 的交付方式；先核对再写 go.mod。
...
- 只声明治理侧真正需要转发的消息与边界。Model 先例是有界列表查询（limit 1–100、无分页元数据），不是全量分页透传。
...
- gnostic operation 描述里写明鉴权、订阅、拒绝语义与错误码（Model 的 `i_model.proto` 是模板）。
...
生成物落在 `api/gen/go/`，不手改。此后 `go.mod` 需要下游 API 依赖（`go mod tidy`；固定版本按 §0.3 核对交付方式）。

## 5. 出站 mTLS 客户端（`internal/data/<entity>_client.go`）

照 [model_client.go](../app/admin/service/internal/data/model_client.go) 的不变量逐条核对：

- **fail-closed 构造**：address 与正 timeout 必填；CA 必须能解析（无证书即报错）；客户端证书加载失败即报错；leaf 的 DNS SAN 必须精确包含 `ani-governance`（这是治理中心自己的身份，不是下游的）。
- **传输**：`tls.Config{MinVersion: TLS13}`；`ServerName` 固定为下游 DNS 名（如 `ani-model-service`）；`grpc.WithDisableServiceConfig()`——本合同没有 DNS TXT 服务发现权威。
...
照 [model_service.go](../app/admin/service/internal/service/model_service.go) 的不变量：
...
- 查询参数**严格白名单**：逐个校验 `transport.FromServerContext` 里的原始 query，未知/重复/空值参数拒绝（Model 拒绝 cursor/keyword/source/capability 注入）。
...
1. **[wiring_ent.go:246](../app/admin/service/cmd/server/wiring_ent.go#L246)** `── register:service ──` 锚点后追加四连（Model 先例 249–260 行）：
...
2. **[rest_server.go:175](../app/admin/service/internal/server/rest_server.go#L175)** `register:param` 锚点后追加形参 `<entity>Service *service.<Name>Service,`。
3. **[rest_server.go:254](../app/admin/service/internal/server/rest_server.go#L254)** `register:route` 锚点后追加 `adminV1.Register<Name>ServiceHTTPServer(srv, <entity>Service)`。
4. **[wiring_ent.go:297](../app/admin/service/cmd/server/wiring_ent.go#L297)** `register:rest-arg` 锚点后把 `<entity>Service` 加进 `NewRestServer` 实参。

漏接由编译器在调用处报错——所以 §4 生成后先 `make build_only` 一轮。

## 8. 模块与权限登记

1. **[module_mapping.go](../pkg/constants/module_mapping.go)**：`"<Name>Service": identityV1.Module_<MODULE>`。漏登记会导致 `business_module` 为 UNSPECIFIED，租户白名单直接拒绝。
2. **Api 表登记**：新增接口明确登记 `(path, method)`、权限和套餐关系，参照 [bootstrap-model-access.sql](../scripts/bootstrap-model-access.sql) 写专项脚本；已在部署的实例上 `SyncApis` 是**清空后全量重建**，不是无损增量——需保留已有 ID 与关联并验证升级授权链。
...
编译与生成只是起点；隔离、鉴权、持久化、故障恢复**不能由 HTTP 200 或静态检查代替**。最小负向用例集（Model 上线时已验证的等价集合）：
